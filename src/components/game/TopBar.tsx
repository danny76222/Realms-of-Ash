...
export function TopBar() {
  ...
  const objectiveWhere = beatHere
    ? loc?.name
    : journal[0]
      ? LOCATIONS[journal[0].target]?.name
      : null;
  ...
  return (
    <>
      <header className="hud-bar sticky top-0 z-20 mb-3">
        <div className="flex flex-wrap items-center gap-x-3 gap-y-2 px-3 py-2">
          ...
          <span className="min-w-0">
            <span className="heading-font block truncate text-xs text-primary">
              {game.heroName}
            </span>
            <span className="pixel-font block text-[8px] uppercase text-muted-foreground">
              <Icon name={GLYPH.place} /> {loc?.name ?? "the road"} · {timeOf(game).label}
            </span>
          </span>
            ...
            {game.party.map((u) => {
              ...
              return (
                <span key={u.id} className="w-16" title={`${u.name}, ${u.hp}/${s.maxHp} hp`}>
                  <span className="pixel-font block truncate text-[8px] uppercase text-muted-foreground">
                    {u.name.split(" ")[0]}
                  </span>
          ...
          <span className="ml-auto flex flex-wrap gap-1">
            ...
            {screen === "map" ? (
              <PixelButton
                size="sm"
                variant="ghost"
                title="Return to where you stand"
                onClick={() => setScreen("location")}
              >
                <Icon name={GLYPH.place} /> Here
              </PixelButton>
            ) : null}
            <PixelButton
              size="sm"
              variant="ghost"
              title="Quests and decisions"
              onClick={() => setMenu(menu === "journal" ? null : "journal")}
            >
              <Icon name={GLYPH.quests} /> Journal
            </PixelButton>
            <PixelButton
              size="sm"
              title="Pause menu (Esc)"
              onClick={() => setMenu(menu === null ? "pause" : null)}
            >
              <Icon name={GLYPH.menu} /> Menu
            </PixelButton>
          </span>
        </div>
        ...
        <div className="objective mx-2 mb-2 flex flex-wrap items-center gap-2 px-2 py-1">
          ...
          {objectiveWhere ? (
            <span className="text-sm text-muted-foreground">at {objectiveWhere}</span>
          ) : null}
          <span
            className="ml-auto pixel-font text-[8px] uppercase"
            style={{ color: tone.allegiance?.color ?? "var(--muted-foreground)" }}
          >
        ...
        <span
          ...
          style={{
            ["--banner-color" as string]:
              tone.allegiance?.color ??
              (tone.mood === "war" ? "var(--destructive)" : "var(--primary)"),
          }}
        />
      </header>
            ...
            <Panel title="Paused" className="surface-wood">
              <div className="flex flex-col gap-2">
                <MenuRow
                  ...
                  label="Resume"
                  variant="default"
                  hint="Back to the game (Esc)"
                  onClick={() => setMenu(null)}
                />
                <MenuRow
                  icon={GLYPH.quests}
                  label="Quest Journal"
                  hint="Work in hand and past decisions"
                  onClick={() => setMenu("journal")}
                />
                <MenuRow
                  icon="lore"
                  label="World Lore"
                  hint="History, the usurper, and each house's goals"
                  onClick={() => {
                    setMenu(null);
                    setScreen("lore");
                  }}
                ...
                <MenuRow
                  ...
                  onClick={() => setMenu("world")}
                />
                <MenuRow
                  icon={GLYPH.save}
                  label="Save / Load"
                  hint="Manage your save slots"
      ...
      {menu === "settings" ? (
        <SettingsPanel className="mb-3" onClose={() => setMenu(null)} />
      ) : null}
      ...
      {menu === "journal" ? (
        ...
        >
          <ul className="space-y-1 text-sm">
            ...
            {journal.length === 0 ? (
              <li className="text-muted-foreground">
                No work in hand. Ask in a village or castle.
              </li>
            ) : null}
          </ul>

          <h3 className="heading-font mb-1 mt-4 text-[10px] text-primary">
            What you have already decided
          </h3>
          <ul className="max-h-56 space-y-1 overflow-y-auto text-sm">
            {[...game.choiceHistory].reverse().map((c, i) => (
              <li
                key={`${c.beatId}${c.choiceId}${i}`}
                className="border-l-2 border-accent bg-background/40 px-2 py-1 text-muted-foreground"
              >
                <span className="pixel-font mr-1 text-[8px] text-muted-foreground">
                  Day {c.day}
                </span>
                {c.summary}
              </li>
            ...
            {game.choiceHistory.length === 0 ? (
              <li className="text-muted-foreground">
                Nothing yet. The realm is still waiting to find out who you are.
              </li>
            ) : null}
          </ul>
          {game.branch ? (
          ...
          {game.marriedTo ? (
            <p className="pixel-font mt-1 text-[9px] text-accent">
              Wed to {NPCS[game.marriedTo]?.name ?? game.marriedTo}.
            </p>
          ) : null}
        </Panel>
      ) : null}
      ...
      {menu === "world" ? (
        ...
        >
          <div className="grid gap-2 md:grid-cols-2">
            ...
            <div>
              <h3 className="heading-font mb-1 text-[10px] text-primary">
                ...
                {FACTION_IDS.flatMap((a, i) =>
                  FACTION_IDS.slice(i + 1).map((b) => {
                    ...
                    return (
                      <li
                        key={`${a}${b}`}
                        className={kind === "war" ? "text-destructive" : "text-accent"}
                      >
