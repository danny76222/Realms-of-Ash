  ...
  return (
    <div className="mx-auto max-w-2xl px-3 pb-10">
      <Panel title="The Ledger Closes">
        <SceneArt
          src={ENDING_ART[ending.id] ?? ART.coronation}
          alt={ending.title}
          className="mb-3"
          height="h-44 sm:h-60"
        />
        ...
        <h2 className="heading-font mt-4 text-[10px] text-muted-foreground">
          The realm at the end
        </h2>
        <ul className="mt-1 grid gap-0.5 text-sm sm:grid-cols-2">
          {FACTION_IDS.map((id) => (
            <li key={id}>
              {FACTIONS[id].name}: standing {game.factions[id].rep}, strength{" "}
              {game.factions[id].strength}
            </li>
          ))}
        </ul>
        ...
        <p className="mt-3 text-sm text-muted-foreground">
          ...
          {dead.length
            ? `${dead.length} did not survive your story.`
            : "Remarkably, everyone important survived."}
        </p>
