...
export const Route = createFileRoute("/auth")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Sign in to Realm of Ash to keep your campaign save slots in the cloud across devices." },
...
function AuthPage() {
  ...
  const submit = async () => {
    ...
    else if (mode === "up") setMsg("Account created. Check your email if confirmation is required, then sign in.");
    ...
    <main className="mx-auto flex min-h-screen max-w-md flex-col justify-center px-4">
      ...
      <Panel title={mode === "in" ? "Sign In" : "Create an Account"}>
        <p className="mb-3 text-sm text-muted-foreground">
          Cloud saves are optional — the game plays fine as a guest, but signing in keeps your slots across devices.
        </p>
        <div className="space-y-2">
          ...
          <input
            ...
            className="w-full border-2 border-border bg-input px-2 py-2 text-foreground outline-none focus:border-primary"
          />
          <PixelButton className="w-full" disabled={busy || !email || !password} onClick={() => void submit()}>
            {mode === "in" ? "Sign In" : "Sign Up"}
          </PixelButton>
          <PixelButton className="w-full" variant="ghost" onClick={() => setMode(mode === "in" ? "up" : "in")}>
            {mode === "in" ? "Need an account?" : "Have an account?"}
          </PixelButton>
          <a href="/" className="pixel-font block pt-1 text-center text-[10px] uppercase text-muted-foreground underline">
            Play as guest
          </a>
        </div>
